"""
LaRes training without human-designed reward initialization.

Run from the LaRes project root:
    python scripts/LaRes_from_scratch.py --env-name='window-close-v2' [options]

CPU training (default): omit ``--cuda``. ``CUDA_VISIBLE_DEVICES=-1`` is set below so
PyTorch does not use a GPU.

Or with config/run.sh (run from project root):
    bash config/run.sh
"""
...
import numpy as np
import random
from lares.rl.arguments import get_args
from lares.rl.sac import sac_agent
...
from lares.core.replay_buffer import replay_buffer
    ...
    while True:
        ...
        for iiii in range(len(responses)):
            ...
            code = (
                "import numpy as np"
                + "\n"
                + "from lares.core.reward_utils import *"
                + "\n"
                + code_string
            )
            ...
            test_code_path = os.path.join(
                _PROJECT_ROOT, "tests", "test_generate_code.py"
            )
...
if __name__ == "__main__":
    ...
    env = make_metaworld_env(args, args.seed)
    ...
    eval_env = make_metaworld_env(args, args.seed + 100)
    ...
    prompt_dir = f"{Ares_ROOT_DIR}/lares/utils/no_init_prompts"
