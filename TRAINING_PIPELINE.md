...
| File | Role | Status |
|------|------|--------|
| `training_pipeline.py` | Main pipeline: all 4 stages + `SymbolicPolicyPipeline` orchestrator | New |
| `test_training_pipeline.py` | 50+ checks across 3 tiers (mock / MetaWorld / LLM) | New |
| `symbolic_policy.py` | Base class for symbolic policies | Existing |
| `policy_generation.py` | LLM generation pipeline | Existing |
| `test_generate_policy.py` | Subprocess validation harness | Existing |
| `replay_buffer.py` | Added `SimpleReplayBuffer` for single-reward storage | Modified |
| `utils/policy_prompts/` | Prompt templates for LLM policy generation | Existing |
| `run_demo.py` | End-to-end demo on reach-v2 | New |
...
```python
from training_pipeline import generate_dataset
...
```python
from training_pipeline import behavioral_cloning
...
```python
from training_pipeline import rl_finetune
